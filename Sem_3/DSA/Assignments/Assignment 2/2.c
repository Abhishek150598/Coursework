...
//structure to define the strorage -------------------------------------------------
...
//function to store the valu of the matrix -------------------------------


void storemat(matrix m_str)
{
	long i;float trm;
	trm=m_str.value[0];
	for(i=1;i<=trm;i++)
...
//function to multiply a spares matrix with a constant --------------------------------------------


matrix constmult(matrix m_const,float c)
{
	matrix m_result;
...
matrix deletehead(matrix m_del,long del)
{
...
matrix addmat(matrix ma1,matrix ma2)
{
	matrix m_add;
	...
	c2=ma2.column[0];
		...
		{
			if(m_add.value[lp_add]==0)
			{
				m_add=deletehead(m_add,lp_add);
				lp_add--;
			}
		}
		return(m_add);
...
void display(matrix m_pr)
{
	...
	{
		for(j=1;j<=cs;j++)
			printf(" %f ",m_array[i][j]);
		printf("\n");
	}
}

//function to multiply the answer--------------------------------------

matrix multmat(matrix m_mul1,matrix m_mul2)
{
	matrix mul_result;
	...
	{
		if(mul_result.value[lp_mul]==0)
		{
			mul_result=deletehead(mul_result,lp_mul);
			lp_mul--;
		}
	}
		return(mul_result);
...
//function to determine the minor of the matrix -------------- 

matrix minor(matrix ma,long a,long b)
{
	matrix m_min=ma;
	long i_minor;float s_minor;
	for(i_minor=1;i_minor<=m_min.value[0];i_minor++)
	{
		if(m_min.row[i_minor]==a || m_min.column[i_minor]==b)
		{
			m_min=deletehead(m_min,i_minor);
			i_minor--;
		}
	}
	for(i_minor=1;i_minor<=m_min.value[0];i_minor++)
	{
		if(m_min.row[i_minor]>a)
			m_min.row[i_minor]--;
		if(m_min.column[i_minor]>b)
			m_min.column[i_minor]--;
	}
	m_min.row[0]--;
	m_min.column[0]--;
	return(m_min);
...
//function to calculate the determine the dterminate of the matrix -------------------------------
...
//function to calculate thye adjoint of the matrix --------------------------------------------

matrix adjoint(matrix m_xy)
{
	matrix mr,m_new;
